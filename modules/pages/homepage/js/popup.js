...
function getDestaques(last, idDest) {
    ...
    xhttp.open("GET", "functions/func-popup.php?cmdEval=getDestaques");
    xhttp.send();
}
...
function getCategoriaDest() {
    ...
    xhttp.open("GET", "functions/func-popup.php?cmdEval=getListaCatsDestaques");
    xhttp.send();
}
...
function getFormacaoDest() {
    ...
    xhttp.open("GET", "functions/func-popup.php?cmdEval=getFormacoes");
    xhttp.send();
}
...
function getBlogDest() {
    ...
    xhttp.open("GET", "functions/func-popup.php?cmdEval=getBlogPosts");
    xhttp.send();
}
            ...
            xhttp.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (resp[0] == "true") {
                        forms = JSON.parse(resp[1]);                        
                        forms.forEach(function (form) {
                            if(form.form_id == form_value) {
                                document.getElementById("destaque-formacao-title").value = form.title;
                                ...
                                document.getElementById("destaque-formacao-img").value = form.img_src;
                                document.getElementById("destaque-formacao-url").value = form.slug;
                            }
                        })
                    } else if (resp[0] == "session_expired") {
                        window.location = "login.php";
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (resp[0] == "true") {
                ...
                if(destaques != '') {
                    ...
                    document.getElementById('destaqueInfo').style.display = 'none';
                    let p = document.createElement("p");
                    p.innerText = 'Só é possível ter um destaque no pop-up de cada vez. Se pretender adicionar um destaque diferente, terá de apagar o existente.';
                    document.querySelector('.card-body').append(p);

                    // if(document.querySelector('.card-body p') != null)
                    ...
                    for (const i in destaques) {
                        if (destaques.hasOwnProperty(i)) {
                            ...
                            map.call(categorias, function (c) {
                                if (c.id == dest.popup_destaque_id) {
                                    cat = c.name;
                                }
                            });
                    ...
                    // console.log('nao tem destaque');
                    document.getElementById('destaqueInfo').style.display = 'flex';
                    document.querySelector('.card-body p').style.display = 'none';
                }
...
$saveDestaqueBtn.click(function () {
    ...
    if ($input_select_id != 'null') {
        ...
        if(titles.includes($form_title) || titles.includes($blog_title)) {
            ...
            xhttp.open("POST", "functions/func-popup.php", true);
            xhttp.send(destaque);
        }
    }
});
...
$deleteDestaqueBtn.click(function (id) {
    ...
    xhttp.open("GET", "functions/func-popup.php?cmdEval=deleteDestaque&id=" + id);
    xhttp.send();
});