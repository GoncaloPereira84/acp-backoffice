...
{
    $sqlCmd = "SELECT * from popup_categorias
    order by name asc";
...
{
    $sqlCmd = "SELECT * from popup_homepage";
...
function getFormacoes()
{
    ...
    $form_query = "SELECT f.id as form_id, 
	f.*
    from formacoes as f
    inner join formacoes_categorias as fc
    on fc.id = f.categoria_id
    order by display_order asc";
    $cats = execQueryMySQL($form_query);
    echo "true||" . json_encode($cats);
}
...
function saveDestaque()
{
    ...
    if ($lastRecord[0]["MAX(display_order)"] == NULL) {
        $sql_insert = "INSERT INTO popup_homepage 
            (title,
            text,
            popup_destaque_id,
            img_src,
...
{
    $sql_delete = "DELETE from popup_homepage 
