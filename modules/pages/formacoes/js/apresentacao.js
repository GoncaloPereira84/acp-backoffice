    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (resp[0] == "true") {
                ...
                $input_content.next().find('.note-editable').html(about_data.text);
                $('.formacoes-intro-text span').text(450 - about_data.text.replace(/(<([^>]+)>)/ig, "").length);
...
$saveFormacaoIntro.click(function () {
    ...
    if ($input_content != '' && $input_content.replace(/(<([^>]+)>)/ig, '').length < 401) {
